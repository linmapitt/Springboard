...
SELECT distinct f.name, CONCAT(m.firstname, ' ', m.surname) AS memname from country_club.Bookings b
...
INNER JOIN Facilities f ON sub.facid = f.facid
INNER JOIN Members m ON sub.memid = m.memid
HAVING cost >30
ORDER BY cost DESC 
